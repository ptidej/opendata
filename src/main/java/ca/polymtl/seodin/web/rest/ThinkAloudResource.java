...
public class ThinkAloudResource {
    ...
    public ResponseEntity<ThinkAloud> createThinkAloud(@RequestBody ThinkAloud thinkAloud) throws URISyntaxException {
        ...
        if (thinkAloud.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new thinkAloud cannot already have an ID")).body(null);
        }
     ...
     * or with status 500 (Internal Server Error) if the thinkAloud couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
