...
public class DiaryResource {
    ...
    public ResponseEntity<Diary> createDiary(@RequestBody Diary diary) throws URISyntaxException {
        ...
        if (diary.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new diary cannot already have an ID")).body(null);
        }
     ...
     * or with status 500 (Internal Server Error) if the diary couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/diaries")
    @Timed
